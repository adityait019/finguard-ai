...
def clean_amount(amt_str: str) -> float:
    """Clean amount string: remove spaces, commas, and quotes.""" 
    if not amt_str or str(amt_str).strip() == "":
        return 0.0
    # Remove quotes, spaces, and commas [web:64]
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name == "read_transaction_file":
        ...
        try:
            ...
            # Use correct column names from your CSV [file:79]
            total_credited = 0.0
            total_debited = 0.0
            merchant_spending = {}

            for t in transactions:
                amt = clean_amount(t.get('AMT', 0))  # [web:59][web:64]
                debit_credit = str(t.get('Debit / Credit', '')).strip()
                description = str(t.get('Description', 'Unknown')).strip()
                
                # Extract merchant name (first word of description) [file:79]
                merchant = description.split()[0] if description else "Unknown"

                if debit_credit == "Credited":
                    total_credited += amt
                else:  # "Debited"
                    total_debited += amt
                    merchant_spending[merchant] = merchant_spending.get(merchant, 0) + amt

            net_balance = total_credited - total_debited

            result = f"""Transaction Summary from {file_name}:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Total Credited: ₹{total_credited:,.2f}
Total Debited: ₹{total_debited:,.2f}
Net Balance: ₹{net_balance:,.2f}

Top Merchants by Spending:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
""" + "\n".join(f"  {merchant}: ₹{amt:,.2f}" 
                for merchant, amt in sorted(merchant_spending.items(), 
                                           key=lambda x: x[1], 
                                           reverse=True)[:10])
